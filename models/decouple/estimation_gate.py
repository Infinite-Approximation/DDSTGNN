...
class EstimationGate(nn.Module):
    ...
    def forward(self, node_embedding_u, node_embedding_d, day_in_week_feat, month_in_year_feat, history_data):
        ...
        batch_size, seq_length, _, _ = day_in_week_feat.shape
        estimation_gate_feat = torch.cat([day_in_week_feat, month_in_year_feat, node_embedding_u.unsqueeze(0).unsqueeze(0)
                                          .expand(batch_size, seq_length,  -1, -1), node_embedding_d.unsqueeze(0).unsqueeze(0)
                                          .expand(batch_size, seq_length,  -1, -1)], dim=-1)
