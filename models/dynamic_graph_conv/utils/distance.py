...
class DistanceFunction(nn.Module):
    def __init__(self, **model_args):
        ...
        self.input_seq_len      = model_args['in_seq_length']
    ...
    def forward(self, X, E_d, E_u, D_W, M_Y):
        # last pooling
        D_W = D_W[:, -1, :, :]
        M_Y = M_Y[:, -1, :, :]
        # dynamic information
        ...
        X1 = torch.cat([dy_feat, D_W, M_Y, emb1], dim=-1)                    # hidden state for calculating distance
        X2 = torch.cat([dy_feat, D_W, M_Y, emb2], dim=-1)                    # hidden state for calculating distance
        X  = [X1, X2]
        adjacent_list = []
        for _ in X:
            Q = self.WQ(_)
