...
class DecoupleLayer(nn.Module):
    def __init__(self, hidden_dim, fk_dim=256, **model_args):
        super().__init__()
        self.estimation_gate = EstimationGate(node_emb_dim=model_args['node_hidden'],
    ...
    def forward(self, history_data: torch.Tensor, dynamic_graph: torch.Tensor, static_graph, node_embedding_u,
                node_embedding_d, day_in_week_feat, month_in_year_feat):
        """decouple layer
        ...
        Args:
            ...
            month_in_year_feat (torch.Parameter): time embeddng M_Y
        ...
        """
        gated_history_data = self.estimation_gate(node_embedding_u, node_embedding_d, day_in_week_feat, 
                                                  month_in_year_feat, history_data)
        dif_backcast_seq_res, dif_forecast_hidden = self.dif_layer(history_data=history_data,
                                                                   gated_history_data=gated_history_data,
                                                                   dynamic_graph=dynamic_graph,
                                                                   static_graph=static_graph)
        # np.save("visualization/data/DDSTGNN/x_inh.npy", dif_backcast_seq_res.cpu())
...
class D2STGNN(nn.Module):
    def __init__(self, **model_args):
        ...
        self._output_dim = model_args['out_seq_length']
        ...
        self.M_i_Y_emb = nn.Parameter(torch.empty(12, model_args['time_emb_dim']))
    ...
    def _prepare_inputs(self, history_data):
        ...
        # time slot embedding
        day_in_week_feat = self.D_i_W_emb[(history_data[:, :, :, num_feat]).type(torch.LongTensor)]  # [B, L, N, d]
        month_in_year_feat = self.M_i_Y_emb[(history_data[:, :, :, num_feat + 1]).type(torch.LongTensor)]
        # traffic signals
        history_data = history_data[:, :, :, :num_feat]
        return history_data, node_emb_u, node_emb_d, day_in_week_feat, month_in_year_feat
    ...
    def forward(self, history_data):
        ...
        # ==================== Prepare Input Data ==================== #
        history_data, node_embedding_u, node_embedding_d, day_in_week_feat, month_in_year_feat = self._prepare_inputs(
            history_data)
        ...
        static_graph, dynamic_graph = self._graph_constructor(node_embedding_u=node_embedding_u,
                                                              ...
                                                              month_in_year_feat=month_in_year_feat)
        ...
        for _, layer in enumerate(self.layers):
            inh_backcast_seq_res, dif_forecast_hidden, inh_forecast_hidden = layer(inh_backcast_seq_res, dynamic_graph,
                                                                                   static_graph, node_embedding_u,
                                                                                   node_embedding_d, day_in_week_feat, 
                                                                                   month_in_year_feat)
